...
from transformer_classifier_char import *
import time
import os
...
def train_model():
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_accuracy:
            torch.save(model.state_dict(), 'best_model_state_trans.bin')
            best_accuracy = val_acc
    toc = time.perf_counter()
    print('Used %.2f seconds' % (toc - tic))
    plot_results(history)
    # evaluate on test data
    model.load_state_dict(torch.load('best_model_state_trans.bin'))
...
def plot_results(history):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('trans_academic.png')
