...
def get_imdb(batch_size, max_length):
    ...
    TEXT.build_vocab(train_ds, vectors=GloVe(name='6B', dim=100, max_vectors=500000))
    ...
    #train_iter = train
    #test_iter = test
    #train_iter = data.BucketIterator(train, batch_size=batch_size, sort_key=None, device=-1)
    #test_iter = data.BucketIterator(test, batch_size=batch_size, device=-1)
    train_iter, test_iter = data.BucketIterator.splits((train_ds, test_ds), batch_size=batch_size, device=-1)
...
if __name__ == "__main__":
    """
        ...
        and counting labels
    """
    train, test, vectors, vocab = get_imdb(1, 128)
