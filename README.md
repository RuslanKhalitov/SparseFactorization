...
The most important packages are torch-sparse and torch-geometric. The batched sparse multiplication described in the main paper is build upon torch_sparse.spmm and torch_geometric.DataLoader functions. Make sure they are installed. 
...
Finally, you will find the data derictory named "lra_release".
...
1,2) The cifar10 and imdb tasks do not require external files, since the datasets for these problems are avaliable via the built-it PyTorch and Keras functional.
...
Please move them to the LRA folder, so the listops_preprocessing.py can convert them to the corresponding torch tensors.
...
We fixed the metadata indices for training/testing/validation splits so we can use the corresponding instances in the inference part to build the attention maps.
...
These files do not require any manual changes. The datasets (tensors) will be uploaded from the folder, the model configurations will be imported from ***psf_training_config.py***.
...
Besides this, each task has its own training configurations, including the *device_id* parameter, which can be set on your own. 

It will allow you to train all tasks in parallel using the same config file. You will see training/testing/validation loss values as well as its accuracies.

### How to create attention maps:
Inside the LRA directory there is a subderictory named **attention_maps**.
